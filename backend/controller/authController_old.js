...
/**
 * Gmail: use explicit smtp.gmail.com:587 + STARTTLS (more reliable than `service: "gmail"` with app passwords).
 * App password = 16 chars from https://myaccount.google.com/apppasswords (2-Step Verification required).
 */
    ...
    try {
        ...
        if (smtpConfigured) {
            try {
                ...
                // Gmail only allows sending as the authenticated account (or verified aliases).
                const fromAddr =
                ...
                let hint =
                    "Could not send the email. Check server logs.";
                if (
                    code === 535 ||
                    ...
                    hint =
                        "Gmail rejected SMTP login. Fix: (1) Turn on 2-Step Verification. (2) Open https://myaccount.google.com/apppasswords — create a password for 'Mail'. (3) Put the 16 characters in backend/.env as SMTP_PASS (remove spaces or use double quotes). (4) SMTP_USER must be that same Gmail address. (5) Restart the backend. Optional: set SMTP_DEBUG=true in .env to see SMTP logs in the terminal.";
                }
...
// ---------------------------------------------------------------------------
// Session / authentication check (requires cookie JWT + isAuth middleware)
// ---------------------------------------------------------------------------
...
export const googleLogin = async (req, res) => {
    try {
        ...
        // In dev we expose the real failure reason so you can fix env/Firebase issues.
        return res.status(500).json({
            success: false,
    ...
    try {
        ...
        if (admin.accountLockedUntil && admin.accountLockedUntil > new Date()) {
            ...
            await logAudit({
                ...
                resourceType: "Auth",
                status: "FAILURE",
                errorMessage: `Account locked. Try again in ${lockTimeRemaining} minutes`,
                ipAddress,
                userAgent,
            });
...
export const adminLogout = async (req, res) => {
    try {
        ...
        return res.status(500).json({ success: false, message: "Admin logout error" });
    }
};
